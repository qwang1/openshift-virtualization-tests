...
def delete_velero_resource(resource, client):
    ...
    resource_kind = shlex.quote(s=resource.kind.lower())
    resource_name = shlex.quote(s=resource.name)
    command = shlex.split(f"bash -c 'echo Y | ./velero delete {resource_kind} {resource_name}'")
